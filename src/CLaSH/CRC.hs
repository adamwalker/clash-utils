...
{-| <https://en.wikipedia.org/wiki/Cyclic_redundancy_check Cyclic redundancy check> 
    ...
    * Serially, one bit at a time, with a linear feedback shift register.

    * Several bits at a time, by unwinding the shift register algorithm n times.

    * All at once using a table which exploits the linearity of the CRC.

    * N bits at a time, also using a table which expolits the linearity of the CRC.
...
{-| Generates the tables for use with `crcTableMultiStep`. -}
makeCRCTableMultiStep
    :: forall n m. (KnownNat n, KnownNat m)
...
crcTableMultiStep
    :: forall m n. (KnownNat m, KnownNat n)
    => Vec (n + 1) (BitVector (n + 1)) -- ^ CRC table generated by `makeCRCTableMultiCycle`
    -> Vec (m + 1) (BitVector (n + 1)) -- ^ CRC table generated by `makeCRCTableMultiCycle`

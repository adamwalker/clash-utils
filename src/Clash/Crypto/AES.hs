...
keyExpander start key = keyState
    ...
    keyState =  delay (errorX "initial keyState") $ mux start (unpack <$> key) (keyScheduleStep <$> (pack <$> rc) <*> keyState)

    rc :: Signal dom (Vec 8 Bool)
    rc =  delay (errorX "initial rc") $ mux start (pure $ unpack 1) (gfDouble <$> rc)
...
aesEncrypt start key block = bundle (cnt .==. 12, pack <$> roundState)
    ...
    roundState = delay (errorX "initial roundState") $ step <$> cnt <*> roundState <*> roundKey <*> block
        where

...
spec = describe "Message receiver" $ do
    ...
    it "Single message packet" $ 
        property $ \pkt ->  
            (length pkt >= 4) ==>
                fmap Clash.toList (listToMaybe (catMaybes (take 20 $ simulate_lazy @System deserialize (stream pkt) :: [Maybe (Vec 4 Int)]))) == Just (take 4 pkt)

    it "Multiple message packet" $ 
        property $ forAll (choose (1, 10)) $ \numMessages -> 
            forAll (replicateM numMessages $ vectorOf 4 arbitrary) $ \messages ->
                fmap Clash.toList (catMaybes (take (4 * 4 * numMessages + 10) $ simulate_lazy @System deserialize (stream (concat messages)) :: [Maybe (Vec 4 Int)])) == messages

    it "Multiple packets with multiple messages" $ 
        property $ forAll (choose (1, 10)) $ \numPackets -> 
            forAll (vectorOf numPackets pktGen) $ \(messages :: [[[Int]]]) ->
                let pkts = map concat messages
                in  fmap Clash.toList (catMaybes (take (4 * length (concat pkts) + 10) $ simulate_lazy @System deserialize ((concat $ map streamNoNop pkts) ++ repeat nopStream) :: [Maybe (Vec 4 Int)])) == (concat messages)

    it "Selected single message" $ 
        property $ \pkt ->  
            (length pkt >= 4) ==>
                \tag -> 
                    fmap Clash.toList (listToMaybe (catMaybes (take 20 $ simulate_lazy @System (deserialize . selectStream (== tag)) (stream $ tag : pkt) :: [Maybe (Vec 4 Int)]))) == Just (take 4 pkt)
            ...
            (length pkt >= 4) ==>
                \tag -> 
                    forAll (suchThat arbitrary (/= tag)) $ \tag' -> 
                        fmap Clash.toList (listToMaybe (catMaybes (take 20 $ simulate_lazy @System (deserialize . selectStream (== tag)) (stream $ tag' : pkt) :: [Maybe (Vec 4 Int)]))) == Nothing

    it "Selected multiple messages" $ 
        property $ forAll (choose (1, 10)) $ \numMessages -> 
            forAll (replicateM numMessages $ vectorOf 4 arbitrary) $ \messages ->
                \tag -> 
                    fmap Clash.toList (catMaybes (take (4 * 4 * numMessages + 10) $ simulate_lazy @System (deserialize . selectStream (==tag)) (stream (tag : concat messages)) :: [Maybe (Vec 4 Int)])) == messages
    ...
    it "Not selected multiple messages" $ 
        property $ forAll (choose (1, 10)) $ \numMessages -> 
            forAll (replicateM numMessages $ vectorOf 4 arbitrary) $ \messages ->
                \tag -> 
                    forAll (suchThat arbitrary (/= tag)) $ \tag' -> 
                        fmap Clash.toList (catMaybes (take (4 * 4 * numMessages + 10) $ simulate_lazy @System (deserialize . selectStream (==tag)) (stream (tag' : concat messages)) :: [Maybe (Vec 4 Int)])) == []
